...
public class TopoAnalyse {
	...
	public boolean analyseRRBySubnet(){
		...
		Entry<String, List<Pair<String,String>>> iIterArr[] = (Entry<String, List<Pair<String,String>>>[])IPADDR_map.entrySet().toArray();
		for(int i = 0;i<iIterArr.length;i++){
			Entry<String, List<Pair<String,String>>> iter = iIterArr[i];
			...
			List<Pair<String,String>> iter_tmp = new ArrayList<Pair<String,String>>();
			for(int j = i+1;j<iIterArr.length;j++){
				Entry<String, List<Pair<String,String>>> ii = iIterArr[j];
			...
			while(bCircle){
				...
				for(Entry<String,Map<String,List<String>>> i :aft_list.entrySet()){
					...
					if(i.getValue().size() > 1){
						...
						Entry<String,List<String>> isetarr[] = (Entry<String,List<String>>[])i.getValue().entrySet().toArray();
						//对source port 循环
						for(int ji=0 ;ji<isetarr.length-1;ji++){
							Entry<String, List<String>> j = isetarr[ji];
							//比较 j,k 端口集合的元素
							for(int ki = ji+1;ki < isetarr.length;ki++){
								Entry<String, List<String>> k = isetarr[ki];
