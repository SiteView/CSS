...
public class TraceAnalyse {
	...
	public TraceAnalyse(Map<String,IDBody> devlist, List<List<String>> routepath_list, List<Pair<Pair<String,String>,String>> routeDestipPairList, List<String> unManagedDevices, ScanParam param){
	...
	public TraceAnalyse(Map<String,IDBody> devlist, List<List<String>> routepath_list, List<Pair<Pair<String,String>,String>> routeDestipPairList, List<String> unManagedDevices){
		this.device_list = devlist;
		this.rtpath_list = routepath_list;
		this.routeIPPairList = routeDestipPairList;
		this.unManagedIPList = unManagedDevices;
		...
		for(Edge i : topo_edge_list){
			...
			if(iConnected == 0){
				...
				for(Edge j : topo_edge_list){
					if(i.getIp_left().equals(j.getIp_left()) || i.getIp_right().equals(j.getIp_right())){
						if(!list_new.contains(j.getIp_right())){
							list_new.add(j.getIp_right());
						}
						if(i.getIp_left().equals(j.getIp_right()) || i.getIp_right().equals(j.getIp_right())){
	...
	public boolean analyseRRByRtPath_Direct(List<Edge> topo_edge_list){
		...
		for(List<String> rp_iter : rtpath_list){
			...
			Pair<Pair<String, String>, String> l_iter = new Pair<Pair<String, String>, String>(), 
					r_iter = new Pair<Pair<String, String>, String>();
	...
	public boolean analyseRRByRtPath(List<Edge> topo_edge_list){
		...
		if(set_conn.size() > 1 && !retracertIp_list.isEmpty()){
			//重新进行路由跟踪
			TraceReader myTR = new TraceReader(scanParam.getRetrytimes(), scanParam.getTimeout(), 30);//hops暂时先固定为30
