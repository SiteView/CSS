...
public class LineFigure extends Polyline implements HandleBounds,
		Introspectable {
	...
	public static Rectangle getPointsBoundsWithArrows(PointList points,
			ArrowType arrowType, int arrowLength, double arrowAngle) {
	...
	private void drawPolyLineWithArrow(Graphics graphics) {
		...
		if (points.size() >= 2) {
			...
			if (arrowType == ArrowType.To || arrowType == ArrowType.Both) {
				// draw end arrow
				PointList arrowPoints = calcArrowPoints(
						points.getPoint(points.size() - 2), endPoint,
						arrowLineLength, ARROW_ANGLE);
				if (fillArrow)
					points.setPoint(arrowPoints.getLastPoint(),
							points.size() - 1);
				arrowPoints.setPoint(endPoint, 2);
				if (fillArrow) {
					if (isEnabled())
						graphics.setBackgroundColor(graphics
								.getForegroundColor());
	 ...
	 */
	/*
	 * @Override public void setBounds(final Rectangle rect) { invalidate();
	 * fireFigureMoved(); repaint(); int correctedWidth = rect.width +
	 * getLineWidth(); int correctedHeight = rect.height + getLineWidth();
	 * Rectangle correctedRectangle = new Rectangle(rect.x, rect.y,
	 * correctedWidth, correctedHeight); super.setBounds(correctedRectangle); }
	 * 
	 * @Override public void setSize(final int w, final int h) { int
	 * correctedWidth = w + getLineWidth(); int correctedHeight = h +
	 * getLineWidth(); super.setSize(correctedWidth, correctedHeight); }
	 * 
	...
	public Rectangle getBounds() {
		...
		if (bounds == null) {
			bounds = getPointsBoundsWithArrows(getPoints(), arrowType,
					arrowLineLength, ARROW_ANGLE);
	...
	protected void outlineShape(final Graphics graphics) {
		...
		if (getFill() > 0) {
			...
			if (horizontalFill) {
				...
				int newW = (int) Math.round(figureBounds.width
						* (getFill() / 100));

				graphics.clipRect(new Rectangle(figureBounds.x, figureBounds.y,
						newW, figureBounds.height));
			} else {
				int newH = (int) Math.round(figureBounds.height
						* (getFill() / 100));
				graphics.clipRect(new Rectangle(figureBounds.x, figureBounds.y
	...
	public void setBounds(Rectangle rect) {
		...
		// figure should be forced to be moved since the bounds of a polyline
		// might be unchanged.
		fireFigureMoved();
	...
	/**
	 * Override this to fix a bug in draw2d polyline: the polyline width should
	 * be considered.
	 * 
	 * @see org.eclipse.draw2d.IFigure#containsPoint(int, int)
	 */
	public boolean containsPoint(int x, int y) {
		int tolerance = (int) Math.max(getLineWidthFloat() / 2.0f, 2);
		LINEBOUNDS.setBounds(getBounds());
		LINEBOUNDS.expand(tolerance, tolerance);
		if (!LINEBOUNDS.contains(x, y))
			return false;
		return shapeContainsPoint(x, y, tolerance)
				|| childrenContainsPoint(x, y);
	}
