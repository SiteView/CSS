...
public class TopologyFigure extends Figure {
	private final static Font DEFAULT_LABEL_FONT = CustomMediaFactory
			.getInstance().getFont(new FontData("Arial", 12, SWT.BOLD));
	...
	public final static double HW_RATIO = (1 - Math.sin(ALPHA) / 2)
			/ (2 * Math.cos(ALPHA));
	...
	public TopologyFigure(org.eclipse.swt.graphics.Rectangle rectangle) {
		this();
	}
		...
		protected void fillShape(Graphics graphics) {
			...
			if (support3D && effect3D) {
				try {
					...
					// pattern = GraphicsUtil.createScaledPattern(graphics,
					// Display.getCurrent(), bounds.x, bounds.y,
	...
	class TopologyLayout extends AbstractLayout {
		...
		public void layout(IFigure container) {
			...
			// bulbBounds.shrink(area.width/2 - scale.getInnerRadius() +
			// GAP_BTW_BULB_SCALE,
			...
			// ramp.setBounds(rampBounds.shrink(area.width/2 -
			// scale.getInnerRadius() - ramp.getRampWidth()+2,
			...
			if (valueLabel != null && valueLabel.isVisible()) {
				...
				valueLabel.setBounds(new Rectangle(area.x
						+ (area.width - labelSize.width) / 2, area.y
						+ area.height - labelSize.height, labelSize.width,
						labelSize.height));
			}
			...
			// Point thumbCenter = new Point(bulbBounds.x +
			// bulbBounds.width*7.0/8.0,
			// bulbBounds.y + bulbBounds.height/2);
			// double valuePosition = 360 -
			// scale.getValuePosition(getCoercedValue(), false);
			// thumbCenter = PointsUtil.rotate(thumbCenter, valuePosition,
			// center);
